        ...
        <Card className="rounded-3xl">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
              Three-phase sizing mode is active. Enter the total number of residential units connected to the transformer.
              The software uses units per phase for ADMD/DF/CF lookup and total units for maximum peak lookup.
            </div>
...
function ResultsPage({ result }: { result: SizingResult | null }) {
  ...
  const summaryCards = result ? [
    ...
    { label: "Residential units total", value: String(result.residential_units) },
    { label: "Units per phase", value: result.residential_units_per_phase.toFixed(2) },
    { label: "ADMD lookup, per phase", value: `${result.ADMD_kW_per_unit.toFixed(3)} kW/unit` },
    { label: "DF lookup, per phase", value: result.DF.toFixed(3) },
    { label: "CF lookup, per phase", value: result.CF.toFixed(3) },
    { label: "Max total peak", value: `${result.max_total_peak_kW.toFixed(2)} kW` },
    { label: "Estimated diversified peak", value: `${result.group_peak_kW.toFixed(2)} kW` },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <Card className="rounded-3xl">
          <CardHeader>
            <CardTitle>ADMD, CF and DF trend</CardTitle>
            <CardDescription>Per-phase ADMD, CF and DF lookup trend for the selected Botswana location class.</CardDescription>
          </CardHeader>
          <CardContent className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <ReLineChart data={trendData}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="admd" stroke="#38bdf8" strokeWidth={2} name="ADMD" />
                <Line type="monotone" dataKey="df" stroke="#10b981" strokeWidth={2} name="DF" />
                <Line type="monotone" dataKey="cf" stroke="#f43f5e" strokeWidth={2} name="CF" />
              </ReLineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        <Card className="rounded-3xl">
          ...
          <CardContent className="space-y-4">
            {summaryCards ? (
              <>
                {summaryCards.slice(0, 4).map((item) => <ResultTile key={item.label} label={item.label} value={item.value} />)}
                <Separator />
...
function getRecommendation(result: SizingResult) {
  ...
  const phaseNote = ` For phase balance, distribute the ${result.residential_units} units as evenly as possible across the three phases (${result.residential_units_per_phase.toFixed(2)} units per phase used for ADMD/DF/CF lookup).`;
  if (loading > 100) return "Selected transformer is overloaded. Move to the next standard rating before approval." + phaseNote;
  if (loading >= 80) return "Loading is high. Consider the next transformer rating if growth or uncertainty is expected." + phaseNote;
  if (loading >= 60) return "Current selection is acceptable. Review future growth before final approval." + phaseNote;
  if (loading >= 40) return "Current selection is in a comfortable operating range for the present design demand." + phaseNote;
  return "Transformer is lightly loaded. Check whether this level of spare capacity is intentional." + phaseNote;
}
...
function exportProjectSummary(result: SizingResult) {
  ...
  const lines = [
    ...
    `Residential units total: ${result.residential_units}`,
    `Units per phase used for ADMD/DF/CF lookup: ${result.residential_units_per_phase.toFixed(2)}`,
    `ADMD lookup, per phase: ${result.ADMD_kW_per_unit.toFixed(3)} kW/unit`,
    `DF lookup, per phase: ${result.DF.toFixed(3)}`,
    `CF lookup, per phase: ${result.CF.toFixed(3)}`,
    `Max total peak for ${result.peak_lookup_units_total} units: ${result.max_total_peak_kW.toFixed(2)} kW`,
    `Estimated diversified peak: ${result.group_peak_kW.toFixed(2)} kW`,
    ...
    `Phase balance note: ${result.phase_balance_note}`,
