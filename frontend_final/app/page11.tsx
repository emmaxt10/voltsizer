              ...
              recentProjects.slice(0, 4).map((project) => (
                <button key={project.id} onClick={() => onOpenProject(project)} className="flex w-full items-center justify-between rounded-2xl border p-4 text-left hover:bg-slate-50">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{project.place} · {project.residentialUnits} units · {project.result ? `${project.result.selected_transformer_kVA.toFixed(0)} kVA` : "No result"}</p>
                  </div>
        ...
        <Card className="rounded-3xl">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
              Lookup-based sizing mode is active for fast project analysis.
            </div>
...
function exportProjectSummary(result: SizingResult) {
  ...
  const lines = [
    ...
    `Group peak: ${result.group_peak_kW.toFixed(2)} kW`,
...
function Workspace({ onLogout }: { onLogout: () => void }) {
  ...
  async function handleRunAnalysis() {
    ...
    try {
      ...
      if (residentialUnits > 100) throw new Error("Residential units cannot exceed 100 with the current lookup table limit.");
